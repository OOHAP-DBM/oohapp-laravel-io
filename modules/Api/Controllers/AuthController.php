...
use App\Notifications\NewDeviceNoti;
use App\Notifications\UserRegisterNoti;
use App\User;
...
class AuthController extends Controller
{
    ...
    {
        $this->middleware('auth:sanctum', ['except' => ['login','register','login_mobile']]);
    }
    ...
    public function login_mobile(Request $req)
    {
        $validator = \Illuminate\Support\Facades\Validator::make($req->all(), [
            'phone' => 'required|exists:users,phone',
            'device_name' => 'required',
            'verification_status'=>'required|in:TRUE,true'
        ]);
        if ($validator->fails()) {
            return $this->sendError('',['errors'=>$validator->errors()]);
        }
        ...
        if (! $user) {
            return $this->sendError(__("User Not Found"),['code'=>'Mobile No not exists']);
        }
       if($user->tokens()->where('name',$req->device_name)->exists()){
       
       }
       else
       {
        // return $user;
        event(new UnusualLoginEvent($user));
        // $user->notify(new NewDeviceNoti);
       }
    //    return $user;
        return [
