...
//        event(new VendorLogPayment($booking));
    ...
    public function confirmPayment(Request $request, $gateway)
    {
        ...
        {
            $res=Booking::where('code',$request->code)->update(['gateway'=>$gateway,'payment_id'=>$request->payment_id,'status'=>'paid','paid'=>$request->paid_amount]);
             event(new BookingCreatedEvent(Booking::where('code',$request->code)->first()));
            return response()->json(['success'=>$res??false,'message'=>'Payment Status Updated Successfully']);
        }
    ...
    {
        $res=Booking::where('code',$request->code)->update(['gateway'=>$gateway,'status'=>'cancel']);
            return response()->json(['success'=>$res??false,'message'=>'Payment Cancelled']);
    }
    }
