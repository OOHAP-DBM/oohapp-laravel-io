...
class ManageSpaceController extends FrontendController
{
    ...
    public function store(Request $request, $id)
    {
        ...
        $hoarding_type = $request->get('hoarding_type') == 1  ? 'OOH' : 'DOOH';

        $category_name = SpaceCategory::where('id', $request->get('category_id'))
            ->value('name');
        ...
        foreach ($surrounding as $entries) {
            foreach ($entries as $entry) {
                if (isset($entry['name'], $entry['content'])) {
                    $surroundingStrings[] = $entry['name'] . ', ' . $entry['content'];
                }
            }
        }
