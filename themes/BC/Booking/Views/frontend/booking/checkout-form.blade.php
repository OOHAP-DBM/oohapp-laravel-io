<div class="form-checkout" id="form-checkout">
        ...
        <div class="row">
            ...
            <div class="col-md-6 field-email">
                <div class="form-group">
                    ...
                    <input type="email" placeholder="{{__("email@domain.com")}}" id="Xemail" class="form-control" value="{{$user->email ?? ''}}" name="email">
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    <label>{{__("Phone")}} <span class="required">*</span></label>
                    <input type="text" placeholder="{{__("Your Phone")}}" class="form-control" id="Xphone" value="{{$user->phone ?? ''}}" name="phone">
                </div>
            </div>
    ...
    <div style="display:none" id="moneyforPaid" totalamount="{{format_money($booking->total - $booking->paid)}}"></div>
    ...
    <div class="form-group">
        <label class="term-conditions-checkbox">
            <input type="checkbox" name="term_conditions"> {{__('I have read and accept the')}} <a target="_blank" href="/page/privacy-policy">{{__('terms and conditions')}}</a>
        </label>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelector('input[name="zip_code"]').addEventListener('blur', function() {
            const zipCode = this.value;
            console.log(zipCode);
            fetch(`/get-location-details/${zipCode}`, {
                    method: 'GET',
                })
                .then(response => {
                    if (!response.ok) {
                    ...
                    if (data.error) {
                        ...
                        if (data.country) {
                            Array.from(countrySelect.options).forEach(option => {
                                if (option.textContent.trim() === data.country.trim()) {
